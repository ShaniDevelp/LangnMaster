      ...
      <aside className="hidden lg:flex flex-col w-60 bg-white border-r border-gray-100 fixed inset-y-0">
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {sidebarExtras.map(item => (
            <Link
              ...
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                pathname === item.href ? 'bg-purple-50 text-[#6c4ff5]' : 'text-gray-600 hover:bg-purple-50 hover:text-[#6c4ff5]'
              }`}
            >
              <span className="text-base">{item.icon}</span>
              {item.label}
        ...
        <div className="px-4 py-4 border-t border-gray-100">
          ...
          <form action={signOut}>
            <button
              type="submit"
              className="w-full text-xs text-gray-500 hover:text-gray-700 text-left py-1 transition-colors"
            >
              Sign out →
            </button>
          </form>
              ...
              return (
                <Link
                  ...
                  className={`flex flex-col items-center gap-0.5 px-3 py-1 transition-colors ${
                    active ? 'text-[#6c4ff5]' : 'text-gray-400 hover:text-[#6c4ff5]'
                  }`}
                >
